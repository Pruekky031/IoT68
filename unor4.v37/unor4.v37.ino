...
void processCommand(String command, bool fromMQTT) {
  ...
  if (commandName == PING)
  {
    ...
    // Return firmware info and connection status
...
// Modified moveServo function to handle multiple servos
void moveServo(int pin, int angle) {
  ...
  // If we found a slot (or it was already assigned), move the servo
  if (servo_index != -1) {
    servo_motors[servo_index].write(angle);
  } else {
